...
CONTAINER_CLAIMS_APP_NAME=$(get_azd_env_value_or_default "CONTAINER_CLAIMS_APP_NAME" "" false)
...
if [ -n "$CONTAINER_CLAIMS_APP_NAME" ]; then
    build_and_push_image "contentprocessorclaimsdemo" "$SCRIPT_DIR/../../src/ContentProcessorClaimsDemo/" "$CONTAINER_CLAIMS_APP_NAME"
    CLAIMS_IMAGE_URI="$ACR_NAME.azurecr.io/contentprocessorclaimsdemo:$AZURE_ENV_IMAGETAG"
    azd env set CLAIMS_DEMO_IMAGE_URI "$CLAIMS_IMAGE_URI" >/dev/null
    echo "Set CLAIMS_DEMO_IMAGE_URI=$CLAIMS_IMAGE_URI in azd env."
else
    echo "CONTAINER_CLAIMS_APP_NAME not set - skipping claims demo image build (deploy infra first)."
fi
