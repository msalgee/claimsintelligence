...
$ContentClaimsDemoPath = Join-Path $ScriptDir "..\..\src\ContentProcessorClaimsDemo"
...
$CONTAINER_CLAIMS_APP_NAME=Get-AzdEnvValueOrDefault -KeyName "CONTAINER_CLAIMS_APP_NAME" -Required $false
...
if ($CONTAINER_CLAIMS_APP_NAME) {
    Build-And-Push-Image "contentprocessorclaimsdemo" "$ContentClaimsDemoPath" $CONTAINER_CLAIMS_APP_NAME
    # Persist the new image URI in azd env so subsequent `azd up` deploys point at it.
    $claimsImageUri = "$ACR_NAME.azurecr.io/contentprocessorclaimsdemo:$AZURE_ENV_IMAGETAG"
    azd env set CLAIMS_DEMO_IMAGE_URI $claimsImageUri | Out-Null
    Write-Host "Set CLAIMS_DEMO_IMAGE_URI=$claimsImageUri in azd env."
} else {
    Write-Host "CONTAINER_CLAIMS_APP_NAME not set - skipping claims demo image build (deploy infra first)."
}
